...
export function generateDaySummary(
  ...
  if (today.mood > 0) {
    if (today.mood >= 4) bullets.push(p + "Humor acima da média — bom sinal!");
    else if (today.mood <= 2) bullets.push(p + "Humor baixo hoje. Cuide-se.");
  ...
  if (adh >= 80) bullets.push(p + "Aderência alta nos hábitos hoje!");
  else if (adh >= 50) bullets.push(p + "Aderência parcial — alguns hábitos ficaram de fora.");
...
export function generateMicroOrientations(
  ...
  const lowMoodNoEx = recent.filter((r) => r.mood > 0 && r.mood <= 2 && r.exerciseMinutes === 0);
...
export function generatePeriodPatterns(records: DailyRecord[], habits: Habit[]): string[] {
  ...
  const goodSleep = records.filter((r) => r.sleepHours >= 7 && r.mood > 0);
  const badSleep = records.filter((r) => r.sleepHours > 0 && r.sleepHours < 7 && r.mood > 0);

  if (goodSleep.length > 2 && badSleep.length > 2) {
    const avgGood = goodSleep.reduce((a, r) => a + r.mood, 0) / goodSleep.length;
    const avgBad = badSleep.reduce((a, r) => a + r.mood, 0) / badSleep.length;
  ...
  if (withEx.length > 2 && noEx.length > 2) {
    ...
    if (adhEx - adhNoEx > 10) {
      patterns.push(`📊 Dias com exercício têm ${Math.round(adhEx - adhNoEx)}% mais aderência nos hábitos.`);
    }
  }
